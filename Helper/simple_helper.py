...
def build_ascii_grid(grid, agent_pos, goal_pos):
    ...
    for y in range(1,grid.height-1):
        ...
        for x in range(1,grid.width-1):
            if (x,y)==tuple(agent_pos): row.append("A")
            elif (x,y)==tuple(goal_pos): row.append("G")
            else:
                c=grid.get(x,y)
                t="empty" if c is None else getattr(c,"type","?")
                row.append("#" if t=="wall" else "~" if t=="lava" else ".")
        rows.append(" ".join(row))
    return "\n".join(rows)

# ---------------- LLM Helper ----------------
class LLMHelper:
    def __init__(self, model_name="qwen2.5", verbose=False):
    ...
    def build_prompt(self, grid_ascii, agent_pos, agent_dir, goal_pos, max_actions, grid):
        ...
        grid_json={
            "positions":{"agent":agent_pos,"goal":goal_pos},
            "agent direction":agent_dir_str,
            "max suggested actions":max_actions,
            "valid actions":VALID_ACTIONS,
            "grid":grid_ascii.split("\n")
        }
        prompt=f"""
You are a controller that must compute the shortest valid action sequence to move an agent on a grid.
The agent moves with 'left','right','forward'. Respect current facing direction.
Input JSON:
{json.dumps(grid_json,indent=2,default=int)}
Respond ONLY with a Python list of actions and NOTHING else.
"""
        if self.verbose: 
            #cprint.info("----PROMPT----\n",prompt)
            cprint.info("DIRECTION:",grid_json["agent direction"])
            cprint.info("GRID:\n",json.dumps(grid_json["grid"],indent=2))
        return prompt

    def suggest_actions(self, grid, agent_pos, agent_dir, goal_pos, max_actions=10):
        ...
        try:
            ...
            if self.verbose: cprint.highlight("--- RAW LLM RESPONSE ---\n",raw)
        ...
        seq=[t for t in tokens if t in VALID_ACTIONS][:max_actions]

        # Safety check
        safe=[]
        sim_pos=list(agent_pos)
        ...
        for s in seq:
            if simulate_actions(sim_pos, sim_dir, [s], grid):
                ...
                elif s=="left": sim_dir=(sim_dir-1)%4
                elif s=="right": sim_dir=(sim_dir+1)%4
            else: break
        return safe
    ...
    for ep in range(episodes):
        ...
        while not done:
            ...
            if use_llm:
                ...
                plan_actions=helper.suggest_actions(env_wrapper.base_env.grid, agent_pos, agent_dir, goal_pos, max_actions=max_actions) if helper else []
...
if __name__=="__main__":
    ...
    stats=train(env_wrapper, episodes=args.episodes, use_llm_helper=args.llm,
